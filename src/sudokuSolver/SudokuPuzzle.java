...
public class SudokuPuzzle {
	...
	public SudokuPuzzle(int[][] givenValues) {
	...
	/*
	 * scanning
	 * "Is it even feasible that this cell is X? given it's row/col/box?
	 */
	...
	/*
	 * Deducing 
	 * "If there is no other cell in this row/col/box that can be X, this cell must be X"
	 */
				...
				for (int possibleVal = 1; possibleVal <= 9; possibleVal++) {
					if (sudokuPuzzle[row][col].isPossibility(possibleVal)) {
						...
						for (int otherCol = 0; otherCol < col; otherCol++) {
							if ( (sudokuPuzzle[row][otherCol].getConfirmedValue() == possibleVal)
							                || sudokuPuzzle[row][otherCol].isPossibility(possibleVal)) {
								originalVal = false; 
								otherCol = col; 
							}
						}
	...
	public void deduceValuesEveryCol() {
		for (int col = 0; col < sudokuPuzzle.length; col++) {
			deduceValues1Col(col);
		}
	}
	public void deduceValues1Col(int col) {
		for (int row = 0; row < sudokuPuzzle.length; row++) {
			if (sudokuPuzzle[row][col].getConfirmedValue() == 0) {
				for (int possibleVal = 1; possibleVal <= 9; possibleVal++) {
					if (sudokuPuzzle[row][col].isPossibility(possibleVal)) {
						...
						for (int otherRow = 0; otherRow < row; otherRow++) {
							if ( (sudokuPuzzle[otherRow][col].getConfirmedValue() == possibleVal)
					                || sudokuPuzzle[otherRow][col].isPossibility(possibleVal)) {
									originalVal = false; 
									otherRow = row; 
							}
						}
						if (originalVal) {
							for (int otherRow = row + 1; otherRow < sudokuPuzzle.length; otherRow++) {
								if ( (sudokuPuzzle[otherRow][col].getConfirmedValue() == possibleVal)
						                || sudokuPuzzle[otherRow][col].isPossibility(possibleVal)) {
										originalVal = false; 
										otherRow = sudokuPuzzle.length; 
								}
							}
						}
						if (originalVal) {
